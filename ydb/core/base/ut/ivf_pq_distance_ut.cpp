...
namespace {
    ...
    Ydb::Table::VectorIndexSettings MakeFloatVectorSettings()
    {
        Ydb::Table::VectorIndexSettings settings;
        settings.set_vector_type(Ydb::Table::VectorIndexSettings::VECTOR_TYPE_FLOAT);
        settings.set_metric(Ydb::Table::VectorIndexSettings::DISTANCE_EUCLIDEAN);
        return settings;
    }
...
Y_UNIT_TEST_SUITE(NIvfPqDistance) {
    ...
    Y_UNIT_TEST(BuildDistanceTable_TrivialCase) {
        ...
        const TString distanceTableBlob = BuildDistanceTable(
            residual, codebook, /*m=*/2, /*nbits=*/1, MakeFloatVectorSettings());
    ...
    Y_UNIT_TEST(BuildDistanceTable_MatchesBruteforce) {
        constexpr ui32 m = 4;
        ...
        const TString distanceTableBlob = BuildDistanceTable(residual, codebook, m, nbits, MakeFloatVectorSettings());
        const TVector<float> distanceTable = DeserializeFloatVector(distanceTableBlob);
    ...
    Y_UNIT_TEST(BuildDistanceTable_NonDivisibleDimAsserts) {
        ...
        UNIT_ASSERT_EXCEPTION(BuildDistanceTable(
            residual, codebook, /*m=*/2, /*nbits=*/1, MakeFloatVectorSettings()), yexception);
    }

    Y_UNIT_TEST(ComputeDistanceViaTable_Trivial) {
        constexpr ui32 m = 3;
        ...
        const double actual = ComputeDistanceViaTable(tableBlob, codesBlob, m, nbits);
        AssertNear(1.5f + 2.25f + 0.5f, static_cast<float>(actual), 1e-6f, "trivial sum");
    }

    Y_UNIT_TEST(ComputeDistanceViaTable_RoundtripWithBuildDistanceTable) {
        constexpr ui32 m = 4;
        constexpr ui32 nbits = 8;
        constexpr ui32 codeCount = 1u << nbits;
        ...
        const TString distanceTable = BuildDistanceTable(
            residual, codebook, m, nbits, MakeFloatVectorSettings());
        ...
        const TString codesBlob = NPackedNBitVector::Serialize(codes, nbits);
        const double actual = ComputeDistanceViaTable(distanceTable, codesBlob, m, nbits);
        ...
        AssertNear(static_cast<float>(expected), static_cast<float>(actual), 1e-3f,
            "ComputeDistanceViaTable must agree with manual partial-sum L2");
    }

    Y_UNIT_TEST(ComputeDistanceViaTable_RejectsBadInput) {
        constexpr ui32 m = 2;
        constexpr ui32 nbits = 8;
        constexpr ui32 codeCount = 1u << nbits;
        ...
        UNIT_ASSERT_EXCEPTION(ComputeDistanceViaTable(tableBlob, codesBlob, m, /*nbits=*/4), yexception);
        UNIT_ASSERT_EXCEPTION(ComputeDistanceViaTable(tableBlob, codesBlob, /*m=*/3, nbits), yexception);

        const TString codesBlob3 = NPackedNBitVector::Serialize(codes3, nbits);
        UNIT_ASSERT_EXCEPTION(ComputeDistanceViaTable(tableBlob, codesBlob3, m, nbits), yexception);
    }

    Y_UNIT_TEST(BuildDistanceTable_SparseCodebookPadsWithInf) {
        const TString residual = SerializeFloatVector({1.f, 2.f, 3.f, 4.f});

        const TString c00 = SerializeFloatVector({0.f, 0.f});
        const TString c01 = SerializeFloatVector({1.f, 2.f});
        const TString c10 = SerializeFloatVector({3.f, 4.f});
        ...
        const TString distanceTableBlob = BuildDistanceTable(
            residual, codebook, /*m=*/2, /*nbits=*/2, MakeFloatVectorSettings());
        const TVector<float> distanceTable = DeserializeFloatVector(distanceTableBlob);
    ...
    Y_UNIT_TEST(BuildDistanceTable_FullyEmptyCodebookSegment) {
        const TString residual = SerializeFloatVector({1.f, 2.f, 3.f, 4.f});
    ...
    Y_UNIT_TEST(ComputeDistanceViaTable_PropagatesInfFromSparseSlot) {
        constexpr ui32 m = 2;
        constexpr ui32 nbits = 8;
        constexpr ui32 codeCount = 1u << nbits;
        ...
        const double finiteResult = ComputeDistanceViaTable(tableBlob, finiteCodesBlob, m, nbits);
        ...
        const double infResult = ComputeDistanceViaTable(tableBlob, infCodesBlob, m, nbits);
